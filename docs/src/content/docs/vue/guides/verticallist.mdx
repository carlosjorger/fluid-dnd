...
### Caret about draggable CSS styles

:::caution
Avoid to setting _css_ `transform` property or `transitions` (`transition-property`, `transition-timing-function`, `transition-duration`, etc) to **draggable** elements because **Fluid DnD** update those properties automatically and can cause unexpected behavior.
:::
